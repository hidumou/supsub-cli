#!/usr/bin/env node
// postinstall.js — downloads the supsub binary for the current platform.
...
function getBinaryName(platform) {
  return platform.platform === 'windows' ? 'supsub.exe' : 'supsub';
}

function getDownloadURL(platform) {
  const ext = platform.platform === 'windows' ? '.zip' : '.tar.gz';
  const asset = `supsub-cli_${VERSION}_${platform.platform}_${platform.arch}${ext}`;
  return `https://github.com/${REPO}/releases/download/v${VERSION}/${asset}`;
}

async function main() {
  // 在源码仓库中跑 pnpm/npm install 时，__dirname 不在 node_modules/@supsub/cli/scripts 下，
  // 此时跳过下载，避免本地开发环境无意义地访问 GitHub Release。
  const isDownstreamInstall = __dirname.includes(path.join('node_modules', '@supsub'));
  if (!isDownstreamInstall) {
    console.log('supsub-cli: skipping postinstall (source install).');
    return;
  }
  ...
  const tmpFile = path.join(os.tmpdir(), `supsub-download-${Date.now()}`);

  // Skip download if binary already matches current version
  if (fs.existsSync(binaryPath)) {
    try {
      const out = execSync(`"${binaryPath}" --version 2>/dev/null`, { encoding: 'utf8' }).trim();
      if (out.includes(VERSION)) {
        console.log(`supsub v${VERSION} already installed, skipping download.`);
        return;
      }
    } catch (_) { /* binary exists but can't run (wrong arch etc.), re-download */ }
  }

  console.log(`Downloading supsub v${VERSION} for ${platform.platform}/${platform.arch}...`);
  console.log(`URL: ${url}`);
  ...
  try {
    ...
    console.log(`supsub installed at ${binaryPath}`);

    // Create symlink in npm global bin directory so `supsub` is on PATH
    createSymlink(binaryPath, binaryName);
  } catch (err) {
    console.error('Failed to install supsub:', err.message);
    process.exit(1);
  } finally {
