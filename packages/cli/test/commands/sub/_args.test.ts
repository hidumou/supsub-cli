...
import {
  ...
  parseSourceId,
  requireExclusive,
} from "../../../src/commands/sub/_args.ts";
...
describe("commands/sub/_args - requireExclusive", () => {
  test("两者都未设置时不抛", () => {
    expect(() =>
      requireExclusive({}, ["all", "unread"], "互斥"),
    ).not.toThrow();
  });

  test("仅其一时不抛", () => {
    expect(() =>
      requireExclusive({ all: true }, ["all", "unread"], "互斥"),
    ).not.toThrow();
    expect(() =>
      requireExclusive({ unread: true }, ["all", "unread"], "互斥"),
    ).not.toThrow();
  });
  ...
  test("两者都为 falsy（空串/0/false）时不抛", () => {
    expect(() =>
      requireExclusive({ all: false, unread: 0 }, ["all", "unread"], "x"),
    ).not.toThrow();
  });
});
