# cli-subscription Specification

## Purpose
TBD - created by archiving change add-cli-subscription-crud. Update Purpose after archive.
## Requirements
...
- **GIVEN** sourceId=42 当前未被订阅
- **WHEN** 执行 `supsub sub add --source-id 42 --type MP -o json`
- **THEN** stdout 输出 `{"success":true,"data":{"message":"订阅成功"}}`，退出码 0

#### Scenario: 重复订阅返回业务错误

- **GIVEN** sourceId=42 已经在 fixture 里被订阅
- **WHEN** 再次执行 `supsub sub add --source-id 42 --type MP`
- **THEN** mock 返回 400 `{ code: "ALREADY_SUBSCRIBED" }`；cli 退出码 1，stderr 显示 message

#### Scenario: --source-id 必须为正整数

- **GIVEN** 用户传入非整数（如 `mp_42` 或 `0`）
- **WHEN** 执行 `supsub sub add --source-id mp_42 --type MP`
- **THEN** 退出码 64，stderr 含 `--source-id 必须是正整数`，未发起 HTTP 请求

#### Scenario: remove 缺失参数

- **GIVEN** 用户漏掉 `--type`
- **WHEN** 执行 `supsub sub remove --source-id 42`
...
- **GIVEN** sourceId=42 有 4 篇未读、3 篇已读
- **WHEN** 执行 `supsub sub contents --source-id 42 --type MP -o json`
...
- **GIVEN** sourceId=42 unreadCount=4，未读文章 art_mp_42_001
- **WHEN** 执行 `supsub sub mark-read --source-id 42 --type MP --content-id art_mp_42_001`
- **THEN** 退出码 0，再次 `sub list --type MP -o json | jq '.data[] | select(.sourceId==42) | .unreadCount'` 输出 3

#### Scenario: --all 标记后 unread 归零

- **GIVEN** sourceId=42 unreadCount=4
- **WHEN** 执行 `supsub sub mark-read --source-id 42 --type MP --all`
- **THEN** 再次 list 时 sourceId=42 的 unreadCount === 0
...
- **WHEN** 执行 `supsub sub contents --source-id 108 --type MP --unread`
